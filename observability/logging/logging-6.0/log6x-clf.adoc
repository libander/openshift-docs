...
application:: Selects logs from all application containers, except those in infrastructure namespaces like `default` and `openshift`.
...
FilterTypeDetectMultiline:: Used for detecting multiline exceptions.
FilterTypeDrop:: Used to drop log records based on certain conditions.
FilterTypeKubeAPIAudit:: Specific to Kubernetes API audit logs.
FilterTypeOpenshiftLabels:: Used for filtering based on OpenShift labels.
FilterTypeParse:: Used for parsing logs.
FilterTypePrune:: Used to prune certain fields from logs.
